...
class CsrfMiddleware
{
    ...
    /**
     * Vérifie la requête POST/PUT/DELETE et bloque si le token est invalide
     * À appeler au début des routes protégées
     */
    public static function handle(): void
    {
        ...
        if (in_array($method, $methodsToProtect, true)) {
            // Récupère le token depuis le formulaire ou le header
            ...
            // Régénérer le token après validation pour éviter les attaques par rejeu
            self::generateToken();
        }
    }

    /**
     * Middleware manuel pour vérifier le CSRF (retourne true/false au lieu de bloquer)
     */
