...
class MakeModelCommand implements CommandInterface
{
    ...
    public function execute(?string $name, array $options = []): void
    {
        ...
        foreach ($foreignKeys as $fk) {
            ...
            $camelRelation = ucfirst($relationName);

            $relationProperties .= "    private ?{$modelName} \${$relationName} = null;\n";

            $methodsLines .= "\n    public function get{$camelRelation}(): ?{$modelName}\n";
            $methodsLines .= "    {\n";
            $methodsLines .= "        return \$this->{$relationName};\n";
            $methodsLines .= "    }\n";

            $methodsLines .= "\n    public function set{$camelRelation}(?{$modelName} \${$relationName}): void\n";
            $methodsLines .= "    {\n";
            $methodsLines .= "        \$this->{$relationName} = \${$relationName};\n";
            $methodsLines .= "    }\n";
        }
...
class {$className}
{
{$propertyLines}{$relationProperties}
...
class {$className}Repository extends AbstractRepository implements {$className}RepositoryInterface
{
    ...
    protected string \$entityClass = {$className}::class;
    ...
    {
        return \$this->persist(\${$camelCaseName});
    }

    public function delete({$className} \${$camelCaseName}): bool
    {
        return \$this->remove(\${$camelCaseName});
    }
}
